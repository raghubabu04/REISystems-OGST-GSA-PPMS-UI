...
export interface PossessionHistoryFileUploadProps {
  type: any;
  filesLimit: number;
  updateAlert: any;
  icn: string;
...
export interface PossessionHistoryFileUploadState {
  files: any[];
  type: any;
...
export class FilesUpload extends React.Component<
  PossessionHistoryFileUploadProps,
  PossessionHistoryFileUploadState
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidUpdate(
    prevProps: Readonly<PossessionHistoryFileUploadProps>,
    prevState: Readonly<PossessionHistoryFileUploadState>,
    snapshot?: any
  ): void {
  ...
  onDrop = (file, index) => {
    ...
    this.inventoryApiService
      .uploadPossessionFile(data)
      .then((response: any) => {
        let request = new FormData();
        request.append("file", file);
        request.append("databaseId", response.data.id);
        request.append("controlNumber", this.props.icn);
        request.append("type", "ICN");
        this.commonAPIService
          ...
          .catch((error: any) => {
            ...
            this.inventoryApiService
              .deletePossessionFiles(deleteAPIPayload)
              .then((response) => {
  ...
  onUpClick = (clickedFile, aboveFile) => {
    ...
    console.log("what is id", clickedFile.id);
    console.log("what is id", aboveFile.id);
    let apiPayload = [
    ...
    this.inventoryApiService
      .updatePossessionItems(apiPayload)
      .then((response) => {
        ...
        ).uploadItemHistory.attachmentOrder;
    ...
    this.inventoryApiService
      ...
      .then((response) => {
        if (newFileList.length !== 0) {
          ...
          this.inventoryApiService
            .updatePossessionItems(updateAPIPayload)
            .then((response) => {
              this.setState({
                files: newFileList,
                filesSelectedToDelete: [],
  ...
  updateImage = async (fileId) => {
    ...
    if (fileToUpdate) {
      ...
      inventoryApiService
        .updatePossessionFile(updateData)
        .then((response: any) => {
          let data = new FormData();
          data.append("file", file);
          data.append("databaseId", fileToUpdate.id);
          data.append("controlNumber", this.props.icn);
          commonAPIService
            .uploadFile(data)
            .then((resp: any) => {
