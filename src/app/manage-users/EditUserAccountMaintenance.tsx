...
import {UserAccountMaintenanceSideNav} from "./UserAccountMaintenanceSideNav";
import PPMSMultiSelect from "../../ui-kit/components/PPMS-multi-select";
import {UserAccount} from "../models/userAccountDTO";
...
export interface EditUserAccountMaintenanceState {
  id: string;
...
interface EditUserAccountMaintenanceProps {
  match: any;
  location: any;
  history: any;
  context: any;
...
export class EditUserAccountMaintenance extends React.Component<EditUserAccountMaintenanceProps,
  EditUserAccountMaintenanceState> {
  ...
  constructor(props: any) {
    ...
    } as EditUserAccountMaintenanceState;
  ...
  */
    ...
    if (permissionArray) {
      permissionArray.permissions.forEach((groupPermission: any) => {
        groupPermission.permissionList.forEach((permission) => {
          let isPresent =
            permission.permissionCode === "FS" && !includesInArray;
          if (!permissionCodes.includes("SM")
            && !permissionCodes.includes("AC")
            && !permissionCodes.includes("NU")
            && !permissionCodes.includes("VO")) {
  ...
  disablePAPermission = () => {
    ...
    if (permissionArray) {
      permissionArray.permissions.forEach((groupPermission: any) => {
        groupPermission.permissionList.forEach((permission) => {
          let isPresent =
          permission.permissionCode === "PA" && !includesInArray;
        if (!permissionCodes.includes("SM")
          && !permissionCodes.includes("AC")
          && !permissionCodes.includes("NU")
          && !permissionCodes.includes("VO")) {
  ...
  setGSAPermissions = () => {
    ...
    permissionCodes.forEach((code) => {
      this.handlePermissionsBasedOnSelected(code, permissionCodes, permissionArray);
    });
    this.setState({
      permissionArray,
    });
  ...
  componentWillMount() {
    ...
    this.userApiService
      .getUserAccountByAccountId(accountId)
      .then((response: any) => {
        let leaInformationDTO = response?.data?.leaUser;
        if (response?.data?.approvingOfficialEmail) {
          this.getApprovingOfficialDetails(response?.data?.approvingOfficialEmail);
        }

        this.setState({
          id: response?.data?.id,
          confirmEmailAddress: response?.data?.emailAddress,
          aoEmail: response?.data?.approvingOfficialEmail,
          fedContractorNonFedRecipientDonee:
            response?.data?.fedContractorNonFedRecipientDonee,
          fiAddress1: leaInformationDTO?.address?.addressLine1,
          fiAddress2: leaInformationDTO?.address?.addressLine2,
          fiAddress3: leaInformationDTO?.address?.addressLine3,
          fiCity: leaInformationDTO?.address?.city,
          fiStateCode: leaInformationDTO?.address?.stateCode,
          fiZip1Code: leaInformationDTO?.address?.zip,
          fiZip2Code: leaInformationDTO?.address?.zipExtension,
          fiState: leaInformationDTO?.address?.stateCode,
        ...
        if (response?.data?.userAccountAddressActivityCodes) {
          const aacCodesJson = response?.data?.userAccountAddressActivityCodes;
          let aacCodeEmpty;
          response?.data?.userAccountAddressActivityCodes.find((o) => {
            aacCodeEmpty = o.aac === "";
          });
          if (aacCodeEmpty && UserUtils.isUserNuo()) {
            this.userApiService.getAgencyBureauCodesAndLongNameForNUO().then(resp => {
              let selectedAgencies = [];
              aacCodesJson.forEach((code) => {
                resp.data.forEach((item) => {
                  if (code.agencyBureauCd === item.code) {
                    selectedAgencies.push({
                      agencyBureau: item.code + "-" + item.longName,
                      id: item.code,
                      isSelected: true,
                    })
                  }
          ...
          } else if (aacCodeEmpty && UserUtils.isSystemAdminUser()) {
            this.commonApiService.getAllAgencyBureaus().then(resp => {
              let selectedAgencies = [];
              aacCodesJson.forEach((code) => {
                resp.data.forEach((item) => {
                  if (code.agencyBureauCd === item.code) {
              ...
              if (!UserUtils.isUserSaspAdmin()) {
                this.commonApiService
                  ...
                  .then((response: any) => {
                    ...
                    this.handleAddAAC(accCodeData.id, accCodeData.primaryInd);
                  })
                  .catch((error: any) => {
  ...
  handleSubmit = (event: any) => {
    ...
    if (this.state.permissionCodes.includes("FI") && (this.state.fiDoneeOrganizationName === "" || this.state.fiDoneeOrganizationName === undefined)) {
      this.setState({
      ...
      validationMessages.push(this.state.fiDoneeOrganizationNameValidationMessage)
    }

    let validateAacCodes =
      !(UserUtils.isUserSaspAdmin() || UserUtils.isUserSaspWithFI()) && this.state.permissionCodes.length > 0 &&
    ...
    if (
      ...
      const data: UserAccount = this.toJSON(this.state);
      ...
      this.userApiService
        ...
        .then((response: any) => {
          ...
          this.props.history.push({
            pathname: `${Paths.editUserPage + "/" + params.userId}`,
          });
        })
        .catch((error: any) => {
          this.setState({
  ...
  validateAacCodes = (validationMessages) => {
    ...
    const addAacCodeMessage = "At least one AAC needs to be added";
    const nuoRelatedagencyBureauMessage = "At least one agency bureau must be selected or At least one AAC needs to be added"
    ...
    if (this.state.aacCodes.length > 0) {
      ...
      if (!this.state.permissionCodes.includes("SM")
        && !this.state.permissionCodes.includes("AC")
        && !this.state.permissionCodes.includes("NU")) {
        this.setState({
        ...
        if (this.state.permissionCodes.includes("NU")
          && this.state.agency?.length === 0
          && this.state.aacCodes?.length === 0) {
          this.setState({
            FormErrorMessage: nuoRelatedagencyBureauMessage,
          });
          validationMessages.push(nuoRelatedagencyBureauMessage);
          return true;
        }
      }
    }
  ...
  toJSON = (state: EditUserAccountMaintenanceState): UserAccount => {
    ...
    state.agency.forEach((o) => {
      selectedAgencyArray.push({
        aac: "",
        agencyBureau: "",
        primaryInd: false,
        agencyBureauCd: o.id,
    ...
    return {
      accountId: accountId,
      userAccountAddressActivityCodes: state.activeAacCodes.length > 0 ? state.activeAacCodes : selectedAgencyArray,
  ...
  handleAddAAC = async (id: any, primaryInd: boolean) => {
    //add validation
    ...
    if (
      ...
      if (!isUserSaspAdminorFI && !this.state.permissionCodes.includes("NU")) {
        aacCodes.push({
      ...
      } else if (this.state.permissionCodes.includes("NU")) {
        if (this.state.aacCode) {
          aacCodes.push({
            aac: this.state.aacCode,
            agencyBureau: this.state.agencyBureauLongName,
            primaryInd: primaryInd,
  ...
  resetPermission = (aacCodes) => {
    ...
    if (
      ...
      this.atLeastOneAACIsInternalAgency(permissionArray, fullListOfPermissionArray, permissionCodes);
  ...
  atLeastOneAACIsInternalAgency = (permissionArray, fullListOfPermissionArray, permissionCodes) => {
  ...
  */
  disablePermissionsForSMACNUOVO = (permissionCode, permissionCodes, permissionArray) => {
    if ((!permissionCodes.includes("HD") && !permissionCodes.includes("VO")) || permissionCodes.length === 1) {
    ...
    if (permissionArray) {
      permissionArray.permissions.forEach((groupPermission: any) => {
        groupPermission.permissionList.forEach((permission) => {
          if (permissionCodes.includes("HD") && (permission.permissionCode === "HD" || permission.permissionCode === "FF" || permission.permissionCode === "VO")) {
            permission.isDisabled = false;
            permission.isSelected = true;
          } else if (permission.permissionCode === permissionCode) {
            permission.isDisabled = false;
            permission.isSelected = true;
          } else {
            permission.isDisabled = true;
  ...
  handlePermissionsBasedOnSelected = (permission, permissionCodes, permissionArray) => {
    ...
    if (selectedPermssions.includes(permission)) {
 ...
 */
  ...
  handleChangePermissions = (event) => {
    ...
    if (event[0].isSelected) {
      this.handlePermissionsBasedOnSelected(event[0].id, permissionCodes, permissionArray);
      if (!permissionCodes.includes(event[0].id)) {
    ...
    this.atLeastOneAACIsInternalAgency(permissionArray, this.state.fullListOfPermissionArray, permissionCodes);
    this.setState({
  ...
  validateForm() {
    ...
    if (this.state.permissionCodes.includes("FI") ||
      this.state.permissionCodes.includes("SP") ||
      this.state.permissionCodes.includes("IF")) {
      this.handleAOEmailChange(this.state.aoEmail, this.state.aoEmailValidationMessage);
    }
  }
  ...
  handleAOEmailChange(value: any, validation: any) {
    this.setState({
      aoEmail: value,
      aoFirstName: "",
      aoLastName: "",
    ...
    if (validation !== undefined && !validation.isInvalid) {
      this.getApprovingOfficialDetails(value);
    }
  };

  getApprovingOfficialDetails(value) {
    this.userApiService
  ...
  render() {
    ...
    let isDisableForHelpDesk = false;
    if (UserUtils.isUserHelpDesk()) {
      isDisableForHelpDesk = true;
    }
                ...
                content: (
                  <div className="usa-accordion--bordered desktop:grid-col-12">
                    ...
                    <PPMSAddress
                      ...
                      addressDisabaled={isDisableForHelpDesk}
                      ...
                      updateAddress1={(value: any, validation: any) => {
                        let valid = value !== undefined && value.length > 0 ? true : false;
                        this.setState({
    ...
    let showNUOwithPermissionSelected = false;
    let showAOInfoWithPermissionsSelected = false;
    if (this.state.permissionCodes.length > 0) {
      showNUOwithPermissionSelected = true;
      showAOInfoWithPermissionsSelected = true;
    }
                ...
                content: (
                  <div className="usa-accordion--bordered desktop:grid-col-12">
                    <PPMSEmail
                      ...
                      updateEmail={this.handleAOEmailChange.bind(this)}
                      maxLength={52}
                    />
                    <div className={"grid-row grid-gap-4"}>
      ...
      saspAdminAAC = [
        {
          ...
          content: (
            <div className="usa-accordion--bordered desktop:grid-col-12">
              <div className={"tablet:grid-col-11 grid-col-12"}>
                <PPMSMultiSelect
                  ...
                  isRequired={false}
                  placeholder={this.state.agency?.length > 0 ? "" : "Select Agency Bureau"}
                  displayValue={"agencyBureau"}
                  selectedValues={this.state.agency}
                  showCheckbox={false}
                  isObject={true}
                  onSelect={this.handleAgencyBureau.bind(this)}
                  onRemove={this.handleAgencyBureau.bind(this)}
                  singleSelect={false}
                  singleSelectAndTypeSearch={true}
                  closeOnSelect={true}
                  isInvalid={false}
                  isValid={true}
                  disable={this.state.aacCodes?.length > 0}
                />
              </div>
              <br />
              <div className="short-table">
              ...
              <PPMSAacAgency
                ...
                aacCodes={this.state.aacCodes}
                isAACCodeDisabled={this.state.agency?.length > 0}
              />
              <br />
              <div className="row">
                <div className="col">
                  <PPMSButton
                    ...
                    isDisabled={!this.state.isAACValid || this.state.agency?.length > 0}
                  />
                </div>
              </div>
            </div>
          ...
          content: (
            <div className="usa-accordion--bordered desktop:grid-col-12">
              ...
              <PPMSAacAgency
                ...
                aacCode={this.state.aacCode}
                agencyBureau={this.state.agencyBureauLongName}
                isAACCodeDisabled={UserUtils.isUserHelpDesk()}
                updateAACCode={(value: any) => {
                  if (value) {
                    this.setState({
                      aacCode: value,
          ...
          <div className={`sticky-wrapper`}>
            <div className={"sticky-inner"}>
              <nav aria-label="Secondary navigation">
                <h3>Form Sections</h3>
                <UserAccountMaintenanceSideNav
                  showAOInfo={showAOInfo}
                  showNUOInfo={showNuoInformation}
                  showFIInfo={showFIInfo}
                  showPermissions={showPermissions}
                  showNUOwithPermissionSelected={showNUOwithPermissionSelected}
                  showAOInfoWithPermissionsSelected={showAOInfoWithPermissionsSelected}
                  hideSectionsForSMandAPO={!this.state.permissionCodes.includes("SM") && !this.state.permissionCodes.includes("AC")}
                />
              </nav>
            </div>
          </div>
        ...
        >
          ...
          >
            <div className="grid-row grid-gap-2">
              <ManageUserButtons
                isSubmitDisabled={UserUtils.isUserHelpDesk() ? true : this.state.isSubmitDisabled}
                isSubmitLoading={this.state.isSubmitLoading}
                cancelFunction={this.handleCancel}
              />
            </div>
            <div className={"action-history-user-account"}>
              <PPMSButton
                ...
                id={"action-history-user-account"}
              />
            </div>
            ...
            {showNUOwithPermissionSelected && (!this.state.permissionCodes.includes("SM") && !this.state.permissionCodes.includes("AC")) ? <>
              <PPMSAccordion bordered={true} items={saspAdminAAC} /><br /> </> : ""}


            {showAOInfoWithPermissionsSelected && (!this.state.permissionCodes.includes("SM") && !this.state.permissionCodes.includes("AC")) ? nuoInformation : ""}
              ...
              <PPMSModal
                body={
                  <ModalActionHistoryContent
                    ...
                    listID={"list-id"}
                    title={this.props.match.params.accountId}
                  />
                }
                ...
                handleSave={""}
                title={"Action History for Account: " + this.props.match.params.accountId}
                centered={true}
                backdrop={"static"}
                label={"Ok"}
...
export default connect(null, mapDispatchToProps)(EditUserAccountMaintenance);
